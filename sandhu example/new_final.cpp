...
#include <sstream>
using namespace std;
string DecisionTable[1000][1000];
string DACtable[1000][1000];
int dac2[1000][1000];
string Attributetable[1000][1000];
string arraycheck[1000];
string input1[1000];
string input2[1000];
string RbacUserRole[1000][1000];
string RbacRoleActivity[1000][1000];
int Rbac_roles=0;
...
void add_att(string a, string b, string c, int d,int row,int col)
{
	...
	for(int i=0;i<Decision_rows;i++)
		{
			if(DecisionTable[i][dec_rules].compare("T") == 0 && DecisionTable[i][0].compare("Subject")!=0 && DecisionTable[i][0].compare("Objects")!=0 && DecisionTable[i][0].compare("Action")!=0 )
				{
...
int main(int argc,char *argv[])
{
	...
	//if(!myans1)cout<<"permission denied by MAC";
	 ...
	 if(!myans4)cout<<"permission denied by RBAC";
