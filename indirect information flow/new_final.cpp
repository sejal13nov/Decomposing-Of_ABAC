...
void copy_mat()
{
for(int i=0;i<user+obj;i++)
	for(int j=0;j<user+obj;j++)
		mat2[i][j]=mat[i][j];
}
int find_indeirectflow()
{
        string user_role_t,user_object_t,user_action_t;
	int s=user;
	int a=obj;
	//int myrow=s+a;
	int count=0;
		...
		for(int j=0;j<Decision_rows;j++)
			...
			if(DecisionTable[j][i].compare("T")==0)
				...
				if(DecisionTable[j][0].compare("Subject")==0)
                                	{
                                 	user_role_t=DecisionTable[j][1];
                              		 }
				if(DecisionTable[j][0].compare("Objects")==0)
                                	{
                                	 user_object_t=DecisionTable[j][1];
                                	}
                       		 if(DecisionTable[j][0].compare("Action")==0)
                                	{
                                 	user_action_t=DecisionTable[j][1];
                                	}
				}
		}
		//cout<<user_role_t<<" "<<user_object_t<<" "<<user_action_t<<endl;
			int a=0,b=0;
			for(int i=1;i<=total_users;i++)
				{
				//cout<<DACtable[i][0]<<user_role_t<<endl;
				if(DACtable[i][0].compare(user_role_t)==0)
					a=i-1;
				}
			for(int i=1;i<=total_objects;i++)
				{
				//cout<<DACtable[0][i]<<"***"<<user_object_t<<endl;
				if(DACtable[0][i].compare(user_object_t)==0)
					b=i-1;
				}
		//cout<<a<<" "<<" "<<b<<endl;
		if(user_action_t.compare("read")==0)
			{
			//g.addEdge(b+user,a+obj);
			mat[b+user][a+obj]=1;
				...
				if(mat2[i][b]==1)
					{
					 for(int j=0;j<user+obj;j++)
						if(mat2[j][i+obj]==1)
							...
							mat2[j][a+obj]=1;
							count++;
							cout<<DACtable[a+1][0]<<" reading "<<DACtable[0][b+1]<<"causes indirect flow \n";}
							}
					}
				...
				if(mat2[b+user][i]==1)
					...
					for(int j=0;j<user+obj;j++)
						if(mat2[i-obj][j]==1)
							...
							if(mat2[a][j]==0)
							{mat2[a][j]=1;count++;
							cout<<DACtable[a+1][0]<<" writing "<<DACtable[0][b+1]<<"causes indirect flow \n";}
							}	
					}	
...
//		}
	 ...
	 for(int i=0;i<s+a;i++)
	   ...
	   //for(int i=0;i<)
           for(int i=0;i<s+obj;i++)
		{
		for(int j=0;j<s+obj;j++)
			if(mat2[i][j])
			{if(j<obj)
				cout<<"Subject "<<DACtable[i+1][0]<<" "<<"can write to"<<DACtable[0][j+1]<<" due to indirect flow\n";
			else
				cout<<"Object "<<DACtable[0][i-user+1]<<" "<<" can be read by"<<DACtable[j-obj+1][0]<<" due to indirect flow\n";}
		}
	//if(ans==0)
		cout<<"indirect information flow";
	return 0;
}
void intial_mac()
{
